...
export default function GraphCanvas() {
  ...
  // ── Organic Node Seeding — Golden-Angle Spiral ───────────────────────────
  // Seed initial positions using a golden-angle spiral so nodes start
  // spread in a circular pattern rather than collapsed at the origin.
  // D3 forces then freely reorganise them into an organic cluster network.
  // NO fx/fy locks are applied here — only the selected focus node is pinned.
  if (filteredNodes !== prevFilteredNodesRef.current || filteredEdges !== prevFilteredEdgesRef.current) {
    ...
    d3NodesRef.current = filteredNodes.map((n, idx) => {
      ...
      const radius = BASE_RADIUS + Math.sqrt(idx + 1) * 28;
      const seedX  = Math.cos(angle) * radius;
      const seedY  = Math.sin(angle) * radius;

      // Preserve coordinates already computed by D3 so the simulation is
      // continuous across re-renders; only apply seed for brand-new nodes.
      const existing = d3NodesRef.current.find(prev => prev.id === n.id);

      // Strip any fx/fy from the source data — no node should be locked
      // except the focus node, which is handled in traversal mode below.
      return {
        ...n,
        x:  existing?.x  ?? seedX,
        y:  existing?.y  ?? seedY,
    ...
    const expandSelectedNode = async () => {
      ...
      try {
        ...
        if (graphMode === 'path') {
          ...
          // Seed nodes around a circle so D3 forces settle them organically.
          // The ONLY pinned node is the selected focus (targetNode at center).
          const seedAroundCenter = (items: any[], ringRadius: number, angleOffset = 0) =>
            items.map((item, idx) => {
              const angle = angleOffset + (idx / Math.max(1, items.length)) * Math.PI * 2;
              return {
                ...item,
          ...
          const prereqNodes     = seedAroundCenter(prereqs,      180, 0);
          const extensionNodes  = seedAroundCenter(extensions,   240, 0.8);
          const appNodes        = seedAroundCenter(applications, 300, 1.6);
          const relatedNodes    = seedAroundCenter(related,      360, 2.4);
          // Pin only the focus node at canvas center
  ...
  // NO row-locking. Pure organic: charge repulsion + link attraction + collision.
  // Nodes settle into natural clusters based purely on their connections.
  useEffect(() => {
    if (!fgRef.current || !d3NodesRef.current.length) return;
    try {
      const fg = fgRef.current;

      // ❶ Remove any center/x/y forces that lock rows
      fg.d3Force('center', null);
      fg.d3Force('x', null);
      fg.d3Force('y', null);

      // ❷ Strong charge repulsion — push nodes apart so they don't overlap
      const chargeForce = fg.d3Force('charge');
      if (chargeForce && typeof chargeForce.strength === 'function') {
        chargeForce.strength(-280);
      }

      // ❸ Link distance — connected nodes attract each other
      const linkForce = fg.d3Force('link');
      if (linkForce && typeof linkForce.distance === 'function') {
        linkForce.distance(110);
      }

      // ❹ Gentle gravity toward canvas center so graph doesn't drift away
      fg.d3Force('gravX', forceX(0).strength(0.035));
      fg.d3Force('gravY', forceY(0).strength(0.035));

      // ❺ Collision detection — prevent node overlaps
      const collideForce = forceCollide((node: any) => {
        try {
          ...
          return 6 + Math.sqrt(degree) * 2 + 36;
        } catch { return 42; }
      });
