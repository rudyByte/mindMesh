...
def format_citation_template(title: str, authors: list, year: int, doi_url: str, style: str) -> str:
    ...
    if not authors:
        ...
        authors_formatted = authors[0]
    elif len(authors) == 2:
        authors_formatted = f"{authors[0]} & {authors[1]}"
    else:
        authors_formatted = f"{authors[0]}, et al."

    # Fallback year and URL
    year_str = str(year) if year else "n.d."
    url_str = doi_url if doi_url else "https://doi.org/"

    # 2. Styles template formatting
    if style.upper() == "APA":
        return f"{authors_formatted}. ({year_str}). {title}. {url_str}"
    elif style.upper() == "MLA":
        return f"{authors_formatted}. \"{title}.\" {url_str}, {year_str}."
    elif style.upper() == "IEEE":
        return f"{authors_formatted}, \"{title},\" {year_str}. Available: {url_str}"
    else:
        return f"{authors_formatted}. {title} ({year_str})."
...
def create_citation(request: CitationCreate):
    ...
    RETURN p.title as title, p.year as year, p.doi as doi, collect(a.name) as authors
    """
    ...
    if not res:
        ...
        if neo4j_client.is_mock() and request.paper_id in neo4j_client.mock_nodes:
            ...
            title = paper.get("title", "Sample Research Paper")
            authors = ["Rudy, B."]
...
def get_citations(style: Optional[str] = Query(None)):
    ...
    if style:
        ...
        RETURN c.id as id, c.style as style, c.formatted_text as formatted_text, p.title as paper_title
        """
