...
def cluster_and_merge_nodes(nodes: list) -> tuple[list, dict]:
    ...
    for node in nodes:
        ...
        for cluster in clusters:
            ...
            if acronym_match or emb_sim >= threshold or (lexical_match and emb_sim >= min(0.82, threshold)):
                cluster.append(node)
                matched = True
                break
        if not matched:
...
def run_extraction_pipeline(doc_id: str, file_bytes: bytes, filename: str, session_id: str):
    ...
    try:
        ...
        def extract_one_chunk(chunk_index: int, chunk_text: str) -> tuple[list, list, bool, bool]:
            if llm_client.provider_unavailable():
        ...
        prereqs = [] if llm_client.provider_unavailable() else llm_client.extract_document_prerequisites(concept_names)
        if prereqs:
